...
def createTeam(indices, isRed,
    ...
    agents = [eval('LeeroyTopAgent' if (index // 2) % 2 == 0 else 'LeeroyBottomAgent')(index) for index in
              indices]
    return agents
...
class ApproximateQAgent(CaptureAgent):
    ...
    def chooseAction(self, state):
        ...
        if (DEBUG):
            ...
            self.newline()
            print
            "AGENT " + str(self.index) + " choosing action!"
        if len(legalActions):
            if util.flipCoin(self.epsilon) and self.isTraining():
                action = random.choice(legalActions)
                if (DEBUG):
                    print
                    "ACTION CHOSE FROM RANDOM: " + action
            else:
                action = self.computeActionFromQValues(state)
                if (DEBUG):
                    print
                    "ACTION CHOSE FROM Q VALUES: " + action
        ...
        if (DEBUG):
            print
            "AGENT " + str(self.index) + " chose action " + action + "!"
        return action
    ...
    """
        if (DEBUG):
            ...
            self.newline()
            print
            "AGENT " + str(self.index) + " updating weights!"
            print
            "Q VALUE FOR NEXT STATE: " + str(self.computeValueFromQValues(nextState))
            print
            "Q VALUE FOR CURRENT STATE: " + str(self.getQValue(state, action))
        ...
        for feature in features:
            ...
            if (DEBUG):
                print
                "AGENT " + str(self.index) + " weights for " + feature + ": " + str(
...
class LeeroyCaptureAgent(ApproximateQAgent):
    ...
    def getCashInValue(self, myPos, gameState, myState):
        ...
        if myState.numCarrying >= self.minPeblletsToCashIn:
